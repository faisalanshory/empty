...
function add_stats(data) {
  ...
  for (var jx = 0; jx < data.length; jx ++) {
    if (data[jx].map) {
      $("#stat_buttons").append("<button type='button' value='" + jx + "'style='flex-wrap: wrap;'" + "'class='btn btn-info' id='" + data[jx].id + "''>" + data[jx].name + "</button> \n");
...
function updateInfo(props) {
  $(".info").html('<h4>' + current_stat.name + '</h4>')
            .append(props ? '<b>' + props.Name + '</b><br />' + props[current_stat.name] + " " + current_stat.unit : 'Hover over a district');
};
